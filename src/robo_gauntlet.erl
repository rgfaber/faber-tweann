...
-define(SPIN_PERIOD, 50).      %% the spinner's fixed firing period, in turns
-define(SPIN_POWER, 5).        %% and its power: a nuisance it can afford
...
observe(#bot{tick = T} = S, #tank{x = MX, y = MY}, Scans) ->
    {_O, _Target, D, {DX, DY}} = nearest(Scans),
    track(S#bot{tick = T + 1, age = 0, dist = D}, MX + DX, MY + DY, T).
...
%% opponent's record.
track(#bot{ex = EX, ey = EY, etick = ET, seen = N} = S, X, Y, T) ->
    S#bot{px = EX, py = EY, ptick = ET,
          ex = X, ey = Y, etick = T,
          seen = min(2, N + 1)}.
...
%%------------------------------------------------------------------- 2. spinner
%% Constant rotation of body, gun and radar, full throttle, and a shot on a fixed
%% period. It ignores its own scans on purpose: it is a moving target with a
%% nuisance gun, not a tracker, and the constant body turn at full throttle is
%% what makes it trace the tight circle that a linear-lead gun finds awkward.
%%
%% The period and the power are set by what a blind gun can AFFORD, and the sums
%% are the reason they look timid. A blind shot at this range connects perhaps
%% one time in eighty, so its expected return is nil and its cost is real: at
%% power 1.0 every ten turns the spinner spends its entire energy bar inside the
%% turn cap and loses to a target that never moves. Half power every fiftieth
%% turn costs about a fifth of the bar over a full match, which a nuisance gun
%% can carry.
spin(#bot{tick = T}) ->
    #intent{turn_body = ?BODY_MAX, turn_gun = ?GUN_MAX, turn_radar = ?SWEEP,
            accel = ?FULL, fire = spin_fire(T rem ?SPIN_PERIOD)}.

spin_fire(0) -> ?SPIN_POWER;
spin_fire(_Phase) -> 0.
...
ram(#bot{seen = 0}, _Me) -> hunt();
ram(#bot{age = A}, _Me) when A > ?LOST -> hunt();
ram(S, Me) ->
    Aim = direct_aim(S, Me),
...
strafe(#bot{seen = 0}, _Me) -> hunt();
strafe(#bot{age = A}, _Me) when A > ?LOST -> hunt();
strafe(S, Me) ->
    Aim = direct_aim(S, Me),
...
snipe(#bot{seen = 0}, _Me) -> hunt();
snipe(#bot{age = A}, _Me) when A > ?LOST -> hunt();
snipe(S, Me) ->
...
lead_aim(S, #tank{x = MX, y = MY} = Me, Power) ->
    Flight = settle(S, Me, bullet_speed(Power), 0, 2),
...
settle(_S, _Me, _Speed, Flight, 0) -> Flight;
settle(S, #tank{x = MX, y = MY} = Me, Speed, Flight, N) ->
    {PX, PY} = predict(S, Flight),
    Range = robo_sim:dist({MX, MY}, {PX, PY}),
    settle(S, Me, Speed, min(?MAX_LEAD, Range div Speed), N - 1).
...
%% so the bot is neither a stationary target nor stuck nose-first into a wall.
hunt() -> #intent{turn_body = 2, turn_radar = ?SWEEP, accel = ?FULL}.
...
ccw({DX, DY}, A) -> robo_sim:cos(A) * DY - robo_sim:sin(A) * DX >= 0.
