...
spawn_neuron_by_type(#neuron{neuron_type = Organelle, id = Id}, _NeuronId)
  when Organelle =:= delay; Organelle =:= leaky ->
    ...
    %% clause below would spawn a plain neuron that computes an activation of
    %% its inputs, so the same genotype would mean one thing here and another
    %% under genotype_to_dag, silently. Raise instead.
    %%
